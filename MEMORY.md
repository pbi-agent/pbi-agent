...
- Tools: built-in `python_exec`, `list_files`, `search_files` removed; use shell (`rg --files`, `find`, `rg -n`) and Python. `read_file` covers images/docs; standalone `read_image` docs removed. `read_web_url` only with `web_search`.
- Tool availability: allow-lists apply to profiles, `run`, commands, and sub-agents via categories (`read`, `write`, `web`, `sub-agent`, `shell`) or built-in names; CLI/command overrides replace profile pairs; empty command lists disable all; `ask_user` stays UI-only/non-configurable.
...
- Command/sub-agent profiles: command frontmatter resolves `model_profile_id` before tool overrides; non-interactive commands keep CLI tool overrides unless command declares tools; parent CLI/command tool overrides flow to profiled sub-agents but parent profile settings do not mask child profiles.
