...
- Tools: built-in `python_exec`, `list_files`, `search_files` removed; use shell (`rg --files`, `find`, `rg -n`) and Python. `read_file` covers images/docs; standalone `read_image` docs removed. `read_web_url` belongs to the `web` allowed-tools group; native web search is separately gated by `web_search`.
- Tool availability: one `allowed_tools` allow-list applies to profiles, `run`, commands, and sub-agents via groups only (`read`, `write`, `web`, `sub-agent`, `shell`); CLI flag is `--allowed-tools`; command/sub-agent frontmatter key is `allowed_tools`; no exact built-in-name allow-list; empty list disables all configurable built-ins; `ask_user` stays UI-only/non-configurable.
