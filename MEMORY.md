...
- Sandbox container names: explicit Docker `--name pbi-agent-{normalized-workspace-folder}`; workspace folder is lowercased, invalid chars/spaces become `-`, trimmed, fallback `workspace`. Duplicate names use Docker conflict behavior.
...
- Fetched `origin/master` (`bcad4d9`) and merged it with local sandbox naming commit `f291c6e`; resolved MEMORY/TODO conflicts, kept upstream session timeline changes/static build artifacts, and validated with `git diff --check` plus focused sandbox pytest.
