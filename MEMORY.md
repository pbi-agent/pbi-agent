...
- Replaced session topbar token/cost pills with a context-window gauge using latest turn `context_tokens` over live runtime `compact_threshold`; live-session API/events/runtime state now carry `compact_threshold`. Validation: `bun run test:web`, `bun run typecheck`, `bun run lint`, `bun run web:build`, `uv run pytest tests/test_web_serve.py -q`, Ruff check/format passed.
- Redesigned session context gauge as a compact donut SVG (no "Context" label) wrapped in a shadcn Tooltip showing absolute usage / threshold / percent on hover; status colors keyed off usage percent. Validation: `bun run test:web`, `bun run typecheck`, `bun run lint`, `bun run web:build` passed.
- Fixed context gauge tooltip clipping with a custom fixed-position tooltip that clamps both axes to 16px viewport gutters; root cause was Radix Popper `shift({ crossAxis: false })`, so `collisionPadding` does not protect the cross-axis border for side/top placements. Validation: `bun run test:web -- UsageBar`, `bun run typecheck`, `bun run lint`, `bun run web:build` passed.
- Documented the Radix/shadcn tooltip edge-clipping lesson in `AGENTS.md` Frontend UI Conventions so future edge-adjacent tooltips use verified gutters or both-axis clamped positioning. Validation: docs-only guidance change; no tests run.
- Updated session context gauge to use live top-level `session_usage.context_tokens` (with final turn usage as fallback) so it advances after each provider/tool-iteration response instead of only at turn end; sub-agent usage events no longer overwrite the top-level gauge/snapshot. Validation: `bun run test:web`, `bun run typecheck`, `bun run lint`, `uv run pytest tests/test_web_serve.py -q`, Ruff check/format for `session_manager.py`, and `bun run web:build` passed.
- Centered the context gauge tooltip token/threshold row while keeping meta text unchanged. Validation: `bun run test:web -- UsageBar` and `bun run web:build` passed (existing chunk-size warning only).
- Added Kanban task prompt image paste support: prompt paste now attaches clipboard image files through existing task image validation/previews, preserves normal text paste, and documents screenshot paste in helper text. Validation: `bun run test:web -- TaskModal`, `bun run typecheck`, and `bun run lint` passed.
- Fixed session timeline Working blocks so completed apply_patch/work runs no longer auto-open; blocks start collapsed and remain open only when that specific block was user-opened. Validation: `bun run test:web -- SessionTimeline`, `bun run test:web`, `bun run lint`, `bun run typecheck`, and `bun run web:build` passed (build emitted existing chunk-size warning).
